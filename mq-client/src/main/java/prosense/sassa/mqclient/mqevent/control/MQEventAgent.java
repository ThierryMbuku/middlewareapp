...
public class MQEventAgent {
    ...
    public static final String idDob = "idDob";
    public static final String creator = "creator";
    public static final String created = "created";

    public void validate(final ObjectNode objectNode) {
        final Set<String> messages = Sets.newHashSet();
        Optional.ofNullable(objectNode.get(transaction))
                .filter(f -> !f.isNull())
                .ifPresent(n -> {
            if (!n.isNumber()) {
                messages.add(transaction + " invalid");
            }
        });
        ...
        Optional.ofNullable(objectNode.get(idDob))
                .filter(f -> !f.isNull())
                .ifPresent(n -> {
            if (!n.isTextual()) {
                messages.add(idDob + " invalid");
            } else {
                if (n.asText().length() > 20) {
                    messages.add(idDob + " max length 20");
                }
            }
        });
    ...
    public MQEvent forCreate(ObjectNode objectNode) {
        ...
        Optional.ofNullable(objectNode.get(transaction)).map(JsonNode::asLong).ifPresent(builder::transaction);
        ...
        Optional.ofNullable(objectNode.get(idDob)).map(JsonNode::asText).ifPresent(builder::idDob);
...
//     	        messages.add(messageId + " mandatory for type PUT or GET");
    ...
    public Map<String, Object> toMap(final MQEvent mqevent) {
        return toFilteredMap(mqevent, ImmutableSet.of(transaction, sequence, messageId, correlation, type, effectedBy, effectedOn, occurred, context, idDob, creator, created));
    }
    ...
    public Map<String, Object> toFilteredMap(final MQEvent mqevent, final Iterable<String> fields) {
        ...
        fields.forEach(f -> {
            if (transaction.equals(f)) {
                map.put(transaction, mqevent.getTransaction());
            }
            ...
            if (idDob.equals(f)) {
                map.put(idDob, mqevent.getIdDob());
            }
            if (creator.equals(f)) {
